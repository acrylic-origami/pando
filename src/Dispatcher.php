...
class Dispatcher<-TState as \State> extends BaseRouter<\Route<TState>> {
	private ImmMap<HttpMethod, ImmMap<string, \Route<TState>>> $routes; // method -> route string -> route
	                                                            //                             └── dependency -> this
	private ?\Route<TState> $default;
	public function __construct(Iterable<\Route<TState>> $flat_routes) {
	...
	public function getRoutes(): ImmMap<HttpMethod, ImmMap<string, \Route<TState>>> {
		return $this->routes;
	}
}