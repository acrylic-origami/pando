...
class MySQLDeltaTransformer<TFrom as Iterable<MySQLDelta>> implements DatabaseDeltaTransformer<MySQLDelta, TFrom, Affected\MySQLIdentifierTree> {
	...
	public function transform(TFrom $v): Affected\MySQLIdentifierTree {
		...
		foreach($v as $delta_obj) {
			...
			switch(key($delta)) {
				...
				case 'UPDATE':
					$aliases = Map{}; // Map<string, \Shufflr\Affected\MySQLIdentifierTree>
	...
	public function _find_side_effect_accumulators(DatabaseDelta $delta_obj): \Shufflr\Affected\MySQLIdentifierTree {
		$accumulators = new \Shufflr\Affected\MySQLIdentifierTree($this->default_db);
